...
<body>
    ...
    <main id="feed">
        <?php
        if (array_key_exists("caption", $_GET)) {
            $q_image = mysqli_query($conn, 'SELECT * FROM photo WHERE caption LIKE "%' . $_GET["caption"] . '%" ');
        } else {
            $q_image = mysqli_query($conn, 'SELECT * FROM photo');
        }

        for ($i = 0; $i < mysqli_num_rows($q_image); $i++) {
            $images = mysqli_fetch_assoc($q_image);
        ?>
            <div class="photo">
